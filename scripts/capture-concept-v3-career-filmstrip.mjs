...
const BASE = process.env.FILMSTRIP_BASE_URL ?? "http://localhost:3000";
...
async function gotoCareer(page, { reduced = false } = {}) {
  ...
  await page.goto(reduced ? PAGE : `${PAGE}?motionOverride=full`, {
  ...
  await page.waitForSelector("[data-career-filmstrip]", { timeout: 20000 });
...
async function selectCompany(page, label) {
  ...
  await page.evaluate((companyId) => {
    const node = document.querySelector(`[data-company-node="${companyId}"]`);
    if (!(node instanceof HTMLElement)) {
      throw new Error(`Missing company node: ${companyId}`);
    }
    node.click();
  }, id);
...
async function checks(page) {
  return page.evaluate((forbidden) => {
    ...
    return {
      hasFilmstrip: Boolean(document.querySelector("[data-career-filmstrip]")),
      yearAxisPresent: /\b2023\b.*\b2024\b.*\b2025\b.*Present/s.test(
        document.querySelector("[data-main-reel]")?.previousElementSibling?.textContent ?? "",
      ),
      eraLabelsAcross: (document.querySelectorAll("[data-era-label]").length > 0),
      alongsideLabel: text.includes("Alongside the main path"),
      modeSelector: text.includes("Main career"),
      active: document
...
async function shot(page, name, report) {
  const file = `concept-v3-filmstrip-${name}.png`;
...
async function main() {
...

  {
    ...
    await shot(page, "desktop-tjar", report);
  ...
  async function recordClip(name, viewport, reduced, run) {
    ...
    if (video) {
      ...
      const dest = path.join(QA, `concept-v3-filmstrip-${name}.webm`);
  ...
  await recordClip("mobile-filmstrip", { width: 390, height: 844 }, false, async (page) => {
    await selectCompany(page, "Eraasoft");
  ...
  const jsonPath = path.join(QA, "concept-v3-filmstrip-capture.json");
